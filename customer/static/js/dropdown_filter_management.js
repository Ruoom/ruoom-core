$('.option_dropdown').on('change', function() {
    ...
    if (option_val !== "") {
        ...
        if (!is_avail) {
            if(ignore_close){
                $('.dropdown_option_tag').last().after("<span class='badge badge-soft-secondary text-wrap dropdown_option_tag dropdown_options' "+data_string+">"+ option_text +"</span>");
            }else{
                $('.dropdown_option_tag').last().after("<span class='badge badge-soft-secondary text-wrap dropdown_option_tag dropdown_options' "+data_string+">"+ option_text +"<span class='badge_close_custom close'>×</span></span>");
            }
            $('.clear-all-btn').css('display', 'inline-block')
            get_customer_class_content('current');
        }
    }
});
